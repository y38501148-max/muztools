...
- **多端通知与热更新**：WebUI 与 Android 共用服务端状态；Android 当前已验证的通知链路为 WebSocket/前台服务，要求应用保持在后台且系统允许通知。客户端保留 FCM 接入代码作为可选通道，但必须成功注册设备 Token 并经过真实设备验证后，才能视为可用的关闭应用推送；中国大陆网络环境下不保证 FCM 稳定送达。应用可通过服务端版本元数据下载安装新版本。
