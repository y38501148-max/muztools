...
v1.3.2 已取消功能审批。既有账号和邀请码注册的新账号均默认拥有功能权限，不再提供 `pending`、`approve-*` 或 `reject-*` 指令。
...
```bash
MUZTOOLS_DATA=/root/muz-tool/data .venv/bin/muz-admin set-version 1.3.2 \
  --code 19 \
  --title 'MuzTool v1.3.2' \
  --message 'Firebase FCM 配置、后台推送链路与双端发布' \
  --apk /tmp/muztools-1.3.2.apk
```
